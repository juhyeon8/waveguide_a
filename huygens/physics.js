...
(function (root, factory) {
  ...
  // 캔버스에 글자를 그릴지. false 면 캔버스에 글자가 하나도 남지 않는다 —
  // ①②③ 밴드 제목과 ① 칸의 "입사파 진행 →" 라벨이 모두 꺼진다. 캡처용 토글이므로
  // 전 밴드에 일관 적용한다 (①만 라벨이 남으면 논문 그림이 어색하다).
  // 진행방향 화살표 **그림**은 남는다 — 글자가 아니라 방향을 나타내는 그림이다.
  // 두 곳에서 false 로 쓴다:
