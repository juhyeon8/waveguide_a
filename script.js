(function () {
  ...
  function computeTransmittance() {
    if (solver.isFloquet) {
      // Floquet 정확해: 전파하는 모든 회절 차수(propagating orders)의 전력 합산
      //   α_m = 2πm/d, κ_m = √(k²−α_m²), k²−α_m²>0 인 차수만 포함
      //   산란진폭 s_m = c·(2i)/(d·κ_m) = pref·(−fCIm + i·fCRe), pref = 2/(d·κ_m)
      //   투과 t_m = (m=0일 때 입사파 1 추가) + s_m,  반사 r_m = s_m
      //   전력 가중 κ_m/k 로  T = Σ (κ_m/k)|t_m|²,  R = Σ (κ_m/k)|r_m|²
      const k   = solver.k;
      const d_m = tabState[0].d_mm / 1000;
      const tpd = TWO_PI / d_m;
      const cRe = solver.fCRe, cIm = solver.fCIm;
      const thr = 1e-9 * k * k;         // κ_m≈0 (Rayleigh 특이점) 상대 임계
      let T = 0, R = 0;
      const orders = [];
      // 전파(open)차수 조건: |α_m| ≤ k  ⇔  |2πm/d| ≤ 2π/λ  ⇔  |m| ≤ d/λ.
      // 루프는 FLOQUET_M(=50)까지 돌지만 아래 kk 필터가 열린 차수(|m|≤d/λ)만 남기므로,
      // 실제 합산에는 −⌊d/λ⌋ … +⌊d/λ⌋ 차수만 들어간다 (λ/d>1이면 m=0 하나뿐).
      for (let m = -FLOQUET_M; m <= FLOQUET_M; m++) {
        const al = m * tpd;
        const kk = k * k - al * al;
        ...
        const sRe = pref * (-cIm);      // s_m 실부
        const sIm = pref * ( cRe);      // s_m 허부
        let tRe = sRe, tIm = sIm;
        if (m === 0) tRe += 1;          // 0차에만 입사파 1
        const w  = kappa / k;           // 진행방향 전력 가중
        const Tm = w * (tRe * tRe + tIm * tIm);
        const Rm = w * (sRe * sRe + sIm * sIm);
        T += Tm; R += Rm;
        orders.push({ m, Tm });
      }
      transmittance = T;
      solver.floquetOrders = orders;    // 회차수별 투과전력 분해 (info 표시용)
      solver.floquetR = R;
      return;
    }
  ...
  function updateInfo() {
    ...
    let floquetLine = "";
    if (activeTab === 0) {
      // 전파차수 개수 + 각 차수의 투과 전력 분해 (예: m=0: 68.5%, m=±1: 각 6.9%)
      const ord = solver.floquetOrders || [];
      const byAbs = {};
      for (const o of ord) {
        const am = Math.abs(o.m);
        if (!byAbs[am]) byAbs[am] = o.Tm;   // ± 쌍은 동일 전력
      }
      const parts = Object.keys(byAbs).map(Number).sort((x, y) => x - y).map(am => {
        const p = (byAbs[am] * 100).toFixed(1);
        return am === 0 ? `m=0: ${p}%` : `m=±${am}: 각 ${p}%`;
      });
      floquetLine =
        `<br>전파차수 <b>${ord.length}개</b> &nbsp;· &nbsp;${parts.join(", ")}`;
      // λ/d = 1.0(= d/λ = 1.0) 부근: ±1차 회절이 열리는 Rayleigh anomaly 임계
      if (Math.abs(dlam - 1) < 0.03) {
        floquetLine +=
          `<br><span style="color:#b5651d">⚠ d/λ ≈ 1.0 : ±1차 회절 임계(Rayleigh anomaly)</span>`;
      }
    }
  ...
  function selfCheck() {
    ...
    // 무한배열(Floquet) 회절차수 에너지 보존 검증: d=30mm, a=1mm 고정
    const savedD = tabState[0].d_mm, savedA = tabState[0].a_mm;
    activeTab = 0; shared.polParallel = true;
    tabState[0].d_mm = 30; tabState[0].a_mm = 1;
    console.log("[검증] 무한배열 에너지보존 (d=30mm, a=1mm):");
    [0.6, 0.8, 1.0, 1.5, 4.07].forEach(ratio => {
      shared.lam_cm = ratio * (tabState[0].d_mm / 10);   // λ[cm] = (λ/d)·d[cm]
      recompute();
      const T = transmittance, R = solver.floquetR || 0;
      const n = (solver.floquetOrders || []).length;
      console.log(
        `  λ/d=${ratio.toFixed(2)} (λ=${shared.lam_cm.toFixed(1)}cm): ` +
        `T=${(T*100).toFixed(1)}%  R=${(R*100).toFixed(1)}%  ` +
        `T+R=${((T+R)*100).toFixed(1)}%  전파차수=${n}개`
      );
    });
    tabState[0].d_mm = savedD; tabState[0].a_mm = savedA;
